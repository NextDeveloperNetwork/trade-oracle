                ...
                {[
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-widest transition-all ${
                      activeIntelligenceTab === tab.id ? "text-indigo-400 opacity-100" : "text-white/20 hover:text-white/40 opacity-100"
                    }`}
                  >
                    {tab.icon}
                    {tab.label}
                      ...
                      {["ALL", "BUY", "SELL"].map(f => (
                        <button 
                          ...
                          className={`text-[8px] font-black uppercase tracking-widest transition-colors ${
                            signalFilter === f ? "text-indigo-400" : "text-white/20 hover:text-white/40"
                          }`}
                        >
                          {f === "INFO" ? "SYSTEM" : f}
                        </button>
                      ...
                      <div key={log.id} className="flex items-center justify-between px-4 py-3 rounded-2xl bg-white/[0.01] border border-white/[0.03] hover:bg-white/[0.05] transition-all group/log">
                        <div className="flex items-center gap-4">
                          ...
                          <div className={`w-1.5 h-1.5 rounded-full ${
                            log.signal === "BUY" ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" : 
              ...
              {activeCoins.map((coin) => {
                ...
                return (
                  <m.div key={coin}
                    ...
                    className={`relative rounded-xl border p-2.5 flex flex-col gap-1 transition-all shadow-lg overflow-hidden group cursor-pointer ${
                      selectedCoin === coin ? "bg-indigo-500/10 border-indigo-500/30" : "bg-[#0d121f]/60 border-white/[0.05]"
                    }`}
                  >
